...
public class RenderSkinnedLootableBody extends RenderBiped {
	...
	public RenderSkinnedLootableBody(RenderManager rm) {
		...
		this.addLayer(new LayerBipedArmor((RendererLivingEntity) this) {
			...
			protected void initArmor() { // get/set armor model?
				this.field_177189_c = new ModelCorpseSkeleton(0.5f, true); // model
																			// base
																			// (child
																			// version?)
				this.field_177186_d = new ModelCorpseSkeleton(1.0f, true); // model
																			// base
			}
		});
	...
	@Override
	protected void preRenderCallback(final EntityLivingBase entity, final float unknown) {
	...
	private ResourceLocation getTexture(final EntityLootableBody e) {
		...
		final Map loadSkinFromCache = minecraft.getSkinManager().loadSkinFromCache(gameprofile); // returned
																									// map
																									// may
																									// or
																									// may
