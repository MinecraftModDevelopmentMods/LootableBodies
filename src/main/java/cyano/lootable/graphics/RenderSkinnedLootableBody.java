...
public class RenderSkinnedLootableBody extends RenderBiped {
	...
	public RenderSkinnedLootableBody(RenderManager rm) {
		...
		this.modelArmorChestplate = new ModelBiped(1.0f) {
			...
			@Override
			public void setRotationAngles(final float f1, final float f2, final float f3, final float f4,
					final float f5, final float f6, final Entity e) {
			}
		};
	...
	@Override
	public void doRender(final EntityLiving p_doRender_1_, final double p_doRender_2_, final double p_doRender_4_,
			final double p_doRender_6_, final float p_doRender_8_, final float p_doRender_9_) {
		...
		doPlayerRender((EntityLootableBody) p_doRender_1_, p_doRender_2_, d3, p_doRender_6_, p_doRender_8_,
				p_doRender_9_);
	}
	...
	public void doLivingRender(final EntityLivingBase p_doRender_1_, final double p_doRender_2_,
			final double p_doRender_4_, final double p_doRender_6_, final float p_doRender_8_,
			final float p_doRender_9_) {
		if (MinecraftForge.EVENT_BUS
				.post(new RenderLivingEvent.Pre(p_doRender_1_, this, p_doRender_2_, p_doRender_4_, p_doRender_6_))) {
			return;
		}
		GL11.glPushMatrix();
		GL11.glDisable(2884);
		...
		/*
		 * this.mainModel.onGround = this.renderSwingProgress(p_doRender_1_,
		 * p_doRender_9_); if (this.renderPassModel != null) {
		...
		try {
			float f2 = this.interpolateRotation(p_doRender_1_.prevRenderYawOffset, p_doRender_1_.renderYawOffset,
					p_doRender_9_);
			f2 = this.interpolateRotation(p_doRender_1_.prevRotationYawHead, p_doRender_1_.rotationYawHead,
					p_doRender_9_);
			...
			final float f4 = p_doRender_1_.prevRotationPitch
					+ (p_doRender_1_.rotationPitch - p_doRender_1_.prevRotationPitch) * p_doRender_9_;
			...
			float f6 = p_doRender_1_.prevLimbSwingAmount
					+ (p_doRender_1_.limbSwingAmount - p_doRender_1_.prevLimbSwingAmount) * p_doRender_9_;
	...
	protected void renderModel(final EntityLivingBase p_renderModel_1_, final float p_renderModel_2_,
		...
		if (!p_renderModel_1_.isInvisible()) {
			...
			this.mainModel.setRotationAngles(p_renderModel_2_, p_renderModel_3_, p_renderModel_4_, p_renderModel_5_,
					p_renderModel_6_, p_renderModel_7_, p_renderModel_1_);
		}
	}

	public void doPlayerRender(final EntityLootableBody body, final double p_doRender_2_, final double p_doRender_4_,
			final double p_doRender_6_, final float p_doRender_8_, final float p_doRender_9_) {
		...
		// super.doRender(body, p_doRender_2_, d3, p_doRender_6_, p_doRender_8_,
		// p_doRender_9_);
	...
	@Override
	protected int shouldRenderPass(final EntityLivingBase p_shouldRenderPass_1_, final int p_shouldRenderPass_2_,
			final float p_shouldRenderPass_3_) {
		final ItemStack itemstack = ((EntityLootableBody) p_shouldRenderPass_1_)
				.armorItemInSlot(3 - p_shouldRenderPass_2_);
		if (itemstack != null) {
			final Item item = itemstack.getItem();
			if (item instanceof ItemArmor) {
				final ItemArmor itemarmor = (ItemArmor) item;
				this.bindTexture(
						RenderBiped.getArmorResource(p_shouldRenderPass_1_, itemstack, p_shouldRenderPass_2_, null));
				...
				modelbiped = ForgeHooksClient.getArmorModel(p_shouldRenderPass_1_, itemstack, p_shouldRenderPass_2_,
						modelbiped);
	...
	private ResourceLocation getTexture(final EntityLootableBody e) {
		...
		// System.out.println(gameprofile.getName()+" cache: (size =
		// "+cache.keySet().size()+")");
		...
		if (cache.containsKey(MinecraftProfileTexture.Type.SKIN)) {
			return minecraft.getSkinManager().loadSkin(
					(MinecraftProfileTexture) cache.get(MinecraftProfileTexture.Type.SKIN),
					MinecraftProfileTexture.Type.SKIN);
		} else {
			// return
			// DefaultPlayerSkin.getDefaultSkin(EntityPlayer.getUUID(gameprofile));
			return TEXTURE_STEVE;

		}
	}

	private float interpolateRotation(final float p_interpolateRotation_1_, final float p_interpolateRotation_2_,
			final float p_interpolateRotation_3_) {
		float f3;
