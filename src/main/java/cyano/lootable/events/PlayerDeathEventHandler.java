...
public class PlayerDeathEventHandler {
	...
	public void playerDeathEvent(PlayerDropsEvent event) {
		...
		// checks to see if event isn't canceled before continuing, preventing
		// errors
		...
		// initiates body creation after passing check
		{
			...
			// System.out.println("Creating corpse with UUID
			// "+corpse.getOwner()+" at
			...
			if (w.getGameRules().getGameRuleBooleanValue("keepInventory") == false) {
				...
				for (int i = 0; i < 4; i++) {
					Map<Integer, Integer> enchantments = player.inventory.armorInventory[i] != null
							? EnchantmentHelper.getEnchantments(player.inventory.armorInventory[i]) : null;
					if (enchantments != null && enchantments.containsKey(LootableBodies.eioSoulboundID)) {
						...
						corpse.setCurrentItemOrArmor(i + 1,
								EntityLootableBody.applyItemDamage(player.getCurrentArmor(i)));
						player.inventory.armorInventory[i] = null;
					}
				}
				for (int i = 0; i < player.inventory.mainInventory.length; i++) {
					Map<Integer, Integer> enchantments = player.inventory.mainInventory[i] != null
							? EnchantmentHelper.getEnchantments(player.inventory.mainInventory[i]) : null;
					if (enchantments != null && enchantments.containsKey(LootableBodies.eioSoulboundID)) {
			...
			// removed profile fetching as it isn't needed, and creates issues
			// when creating bodies(duplicating them if Witchery is installed,
			// and player is vampire*)
	...
	static boolean deepEquals(ItemStack a, ItemStack b) {
		return ItemStack.areItemStacksEqual(a, b) && ItemStack.areItemStacksEqual(a, b)
