...
public class EntityLootableBody extends net.minecraft.entity.EntityLiving implements IInventory {
	...
	private void updatePlayerProfile() {
		...
		GameProfile field_152110_j = MinecraftServer.getServer().getPlayerProfileCache()
				.getGameProfileForUsername(owner.getName());
		if (field_152110_j == null) {
			return;
		}
		if (Iterables.getFirst((Iterable) field_152110_j.getProperties().get("textures"), (Object) null) == null) {
			field_152110_j = MinecraftServer.getServer().getMinecraftSessionService()
					.fillProfileProperties(field_152110_j, true);
		}
	...
	private GameProfile getGameProfileFromName(String name) {
		...
		return new GameProfile((UUID) null, name); // may not be fully
													// initialized
	}
	...
	public void onEntityUpdate() {
		...
		if (vacuumTime < VACUUM_TIMELIMIT) {
			// vacuum up loose items (which may be dropped by other mods that
			// give expanded inventories)
			if (!this.worldObj.isRemote && this.notFull()) {
				...
				List<Entity> ae = this.worldObj.getEntitiesWithinAABB(EntityItem.class,
						AxisAlignedBB.getBoundingBox(x1, y1, z1, x2, y2, z2));
				if (!ae.isEmpty()) {
					for (int n = ae.size() - 1; n >= 0; n--) {
						Entity e = ae.get(n); // old-school for-loop in reverse
												// direction in case there are
	...
	public boolean attackEntityFrom(final DamageSource src, float amount) {
		...
		if (flag) {
			...
			if (entity != null) {
				double d1;
				double d2;
				for (d1 = entity.posX - this.posX, d2 = entity.posZ - this.posZ; d1 * d1
						+ d2 * d2 < 1.0E-4; d1 = (Math.random() - Math.random())
								* 0.01, d2 = (Math.random() - Math.random()) * 0.01) {
				}
	...
	@Override
	protected void damageEntity(final DamageSource src, float amount) {
		// System.out.println("Corpse hit by "+(src.getEntity() == null ?
		// "unknown" : src.getEntity().getName())+" (instance of
		// "+src.getClass().getName() +") for "+amount + " damage");
		...
		if (src.getEntity() != null && src.getEntity() instanceof EntityPlayer
				&& ((EntityPlayer) src.getEntity()).getHeldItem() != null) {
		...
		if (this.hurtByAll)
			super.damageEntity(src, amount);
		if (src.getEntity() != null && src.getEntity() instanceof EntityLivingBase && this.hurtByWeapons)
			super.damageEntity(src, amount);
		if (src.isFireDamage() && this.hurtByFire)
			super.damageEntity(src, amount);
		if (src.isExplosion() && this.hurtByBlast)
			super.damageEntity(src, amount);
		if (src == DamageSource.fall && this.hurtByFall)
			super.damageEntity(src, amount);
		if (src == DamageSource.cactus && this.hurtByCactus)
			super.damageEntity(src, amount);
		if (src == DamageSource.inWall && this.hurtByBlockSuffocation)
			super.damageEntity(src, amount);
		if (this.hurtByOther)
			super.damageEntity(src, amount);
	...
	public void jumpOutOfWall() {
		...
		vector[1] *= normalizer;
		vector[2] *= normalizer;
		Block block = worldObj.getBlock((int) (this.posX + vector[0]), (int) (this.posY + vector[1]),
				(int) (this.posZ + vector[2]));
		if (!(block.getMaterial().blocksMovement())) {
			this.setPosition((int) (this.posX + vector[0]) + 0.5, (int) (this.posY + vector[1]),
					(int) (this.posZ + vector[2]) + 0.5);
			return;
		}
	 ...
	 *         the item was not taken up (means that it was partially stacked
	 *         and the remainder was returned)
	 */
	 ...
	 * 
	 * @param dest
	 *            ItemStack to stack into
	 * @param src
	 *            ItemStack we want to add
	...
	ItemStack stackItemStacks(ItemStack dest, ItemStack src) {
		if (src.stackSize == 0)
			return null;
		// System.out.println("Stacking "+src.toString()+" into
		// "+dest.toString());
	...
	public static ItemStack applyItemDamage(ItemStack itemstack) {
		if (additionalItemDamage == 0)
			return itemstack;
		if (itemstack != null && itemstack.isItemStackDamageable() && itemstack.getItem().isDamageable()
				&& (!itemstack.isStackable()) && (!itemstack.getItem().getHasSubtypes())
