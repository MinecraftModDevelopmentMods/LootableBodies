...
public class EntityLootableBody extends net.minecraft.entity.EntityLiving implements IInventory {
	...
	private void updatePlayerProfile() {
		this.owner = net.minecraft.tileentity.TileEntitySkull.updateGameprofile(this.owner); // fills
																								// in
																								// the
																								// missing
																								// information
	...
	private GameProfile getGameProfileFromName(String name) {
		...
		return net.minecraft.tileentity.TileEntitySkull.updateGameprofile(gp); // fills
																				// in
																				// the
																				// missing
																				// information
	...
	public void onEntityUpdate() {
		...
		if (vacuumTime < VACUUM_TIMELIMIT) {
			...
			if (!this.worldObj.isRemote && this.notFull()) {
				...
				List<EntityItem> ae = this.worldObj.getEntitiesWithinAABB(EntityItem.class,
	...
	public boolean attackEntityFrom(final DamageSource src, float amount) {
		...
		/*
		 * if (!ForgeHooks.onLivingAttack(this, p_attackEntityFrom_1_,
		 * p_attackEntityFrom_2_)) { return false; }
		 */
	...
	public static ItemStack applyItemDamage(ItemStack itemstack) {
		...
		// Damn sloppy modders have metadata items that retrun true when you
		// call itemstack.isItemStackDamageable()
		// The following are extra checks to really confirm that an item is
		// damageable
				&& itemstack.getItem().isDamageable() && (!itemstack.isStackable())
				&& (!itemstack.getItem().getHasSubtypes()) && (itemstack.getItem().getMaxDamage() > 0)) {
