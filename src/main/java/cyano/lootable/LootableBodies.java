...
public class LootableBodies {
	...
	public void preInit(FMLPreInitializationEvent event) {
		// Do stuff in pre-init phase (read config, create blocks and items,
		// register them)
		// load config
		...
		EntityLootableBody.hurtByBlockSuffocation = config.getBoolean("hurt_by_block_suffocation", "corpse damage",
				false, "If true, corpses will be damaged by being stuck inside a block.");
		...
		EntityLootableBody.invulnerable = !or(EntityLootableBody.hurtByAll, EntityLootableBody.hurtByBlast,
				EntityLootableBody.hurtByBlockSuffocation, EntityLootableBody.hurtByCactus,
				EntityLootableBody.hurtByFall, EntityLootableBody.hurtByFire, EntityLootableBody.hurtByOther,
		...
		corpseAuxilleryInventorySize = Math
				.max(config.getInt("corpse_inventory_size", "options",
		...
		corpseDecayTime = Math.max(parseTimeInSeconds(decayTime), 2) * 20; // 2
																			// second
																			// minimum
	...
	private void registerEntity(Class entityClass) {
		...
		EntityRegistry.registerModEntity(entityClass, idName, entityIndex++/* id */, this, 64/* trackingRange */,
				10/* updateFrequency */, true/* sendsVelocityUpdates */);
	}
